...
const registerUser = async function (req, res) {
    try {
        ...
        // role validation 
        // if all these 3 not here then return 
...
const loginUser = async function (req, res) {
    try {
        ...
        return res.status(400).send({ status: false, msg: "you can pass only two keys in request body" })
        const { email, password } = requestBody


        if (!validator.isValid(email)) 
        return res.status(400).send({ status: false, message: ` Key Name : 'email' You can pass only a valid email. Make sure you can not pass only key name or a blank key` })
        ...
        return res.status(400).send({ status: false, message: ` Key Name : 'password' You can pass only a valid password more than 8 character and less than 15 character. Make sure you can not pass only key name or a blank key` })
        ...
        if (!matchPassword) 
        return res.status(404).send({ status: false, message: ` Key Name : 'password' Your password not match try again` })


        const tokenSet = await generateToken.generateToken(user)
        if (!tokenSet) 
        return res.status(404).send({ status: false, message: "There is an error occure to generate token. more details move on console" })
...
const getUser = async function (req, res) {
    try {
        ...
        if (validator.isValidRequestBody(req.query))
         return res.status(400).send({ status: false, msg: "can not pass request query. query is blocked" })
       
         if (validator.isValidRequestBody(req.body))
         return res.status(400).send({ status: false, msg: "can not pass request body. body is blocked" })
